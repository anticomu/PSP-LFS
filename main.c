...
void draw_cube(unsigned int color) {
    ...
    for(int i=0; i<36; i++) {
        ...
        // Vertici del cubo (semplificati per il test)
        vertices[i].x = (i%2 == 0) ? -1.0f : 1.0f;
        vertices[i].y = (i%3 == 0) ? -1.0f : 1.0f;
        vertices[i].z = (i%4 == 0) ? -1.0f : 1.0f;
    }
...
int main() {
    ...
    while(1) {
        ...
        if (current_state == STATE_MENU) {
            ...
            pspDebugScreenPrintf(menu_idx == 0 ? "> GARA" : "  GARA");
            pspDebugScreenSetXY(18, 11);
            pspDebugScreenPrintf(menu_idx == 1 ? "> SELEZIONE AUTO" : "  SELEZIONE AUTO");

            if ((pad.Buttons & PSP_CTRL_DOWN) && !(last_btn & PSP_CTRL_DOWN)) {
                menu_idx++;
            }
            if ((pad.Buttons & PSP_CTRL_UP) && !(last_btn & PSP_CTRL_UP)) {
                menu_idx--;
            }
            if (menu_idx > 1) menu_idx = 0;
            ...
            pspDebugScreenSetXY(15, 5);
            pspDebugScreenPrintf("--- SELEZIONA AUTO ---");
            for(int i=0; i<3; i++) {
            ...
            if (player.car_idx > 2) player.car_idx = 0;
            if (player.car_idx < 0) player.car_idx = 2;
            
            if ((pad.Buttons & PSP_CTRL_CIRCLE) && !(last_btn & PSP_CTRL_CIRCLE)) {
            ...
            if (pad.Lx < 80) player.angle -= 0.05f;
            if (pad.Lx > 170) player.angle += 0.05f;
            player.speed *= 0.97f;
            ...
            sceGumLoadIdentity();
            ScePspFVector3 cam_pos = { player.x - sinf(player.angle)*5, 2.0f, player.z - cosf(player.angle)*5 };
            ScePspFVector3 cam_look = { player.x, 0, player.z };
            ...
            pspDebugScreenPrintf("KM/H: %d", (int)(player.speed * 200));
